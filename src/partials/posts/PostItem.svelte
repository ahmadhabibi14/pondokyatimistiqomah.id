<script lang="ts">
	import type { WPAttachment, WPPost } from '@/types/post';
	...
	const wpFeaturedMedia: WPAttachment[] | undefined = post?._embedded?.['wp:featuredmedia'] as
		| WPAttachment[]
		| undefined;
...
>
	...
	<div class="flex flex-col gap-1 md:gap-2 md:px-4 md:pb-4">
		<h2
			class="text-base md:text-xl font-semibold group-hover:underline line-clamp-3
      md:leading-6 leading-5"
		...
		<time
			class="text-qc text-sm hidden md:flex flex-row gap-1.5 items-center"
			datetime={post.date_gmt + 'Z'}
		>
			<Icon src={RiSystemTimeLine} size="16" className="inline-block fill-qc" />
