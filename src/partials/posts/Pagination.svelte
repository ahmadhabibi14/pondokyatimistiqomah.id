...
<div
	class="flex flex-row gap-2 justify-center items-center font-semibold text-xs mt-4 p-3 bg-white rounded-xl w-fit mx-auto"
>
	...
	<div class="flex flex-row items-center gap-2">
		{#if !paginationShow || paginationShow.length == 0}
			<button class="cursor-not-allowed py-2 px-3 bg-zinc-100 rounded-md" title="No page">
				1
			</button>
		{/if}
		{#each paginationShow as i (i)}
			<button
				onclick={() => OnToPage(i)}
				disabled={currentPage == i}
				class="cursor-pointer py-2 px-3 disabled:cursor-not-allowed {currentPage === i
					? 'bg-istq/20 text-istq'
					: 'hover:bg-zinc-100'} rounded-md"
				title={`Go to page ${i}`}>{i}</button
			>
		{/each}
	</div>
	<div class="flex flex-row items-center gap-0">
